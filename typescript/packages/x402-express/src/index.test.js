...
vi.mock("x402/shared", async (importOriginal) => {
    ...
    return {
        ...
        findMatchingRoute: vi
            .fn()
            .mockImplementation((routePatterns, path, method) => {
            if (!routePatterns)
                return undefined;
...
describe("paymentMiddleware()", () => {
    ...
    it("should return 402 if payment verification fails", async () => {
        ...
        exact.evm.decodePayment.mockImplementation(() => {
    ...
    it("should handle settlement after response", async () => {
        ...
        const endArgs = [];
        mockRes.end.mockImplementation((...args) => {
            endArgs.push(args);
        });
        await middleware(mockReq, mockRes, mockNext);
        expect(exact.evm.decodePayment).toHaveBeenCalledWith(encodedValidPayment);
    ...
    it("should handle settlement failure before response is sent", async () => {
        ...
        mockVerify.mockResolvedValue({ isValid: true });
        mockSettle.mockRejectedValue(new Error("Settlement failed"));
        await middleware(mockReq, mockRes, mockNext);
        expect(mockRes.status).toHaveBeenCalledWith(402);
        expect(mockRes.json).toHaveBeenCalledWith({
            x402Version: 1,
    ...
    describe("session token integration", () => {
        it("should pass sessionTokenEndpoint to paywall HTML when configured", async () => {
            ...
            const middlewareWithPaywall = paymentMiddleware(payTo, routesConfig, facilitatorConfig, paywallConfig);
            mockReq.headers = {
                accept: "text/html",
                "user-agent": "Mozilla/5.0",
            };
            await middlewareWithPaywall(mockReq, mockRes, mockNext);
            expect(getPaywallHtml).toHaveBeenCalledWith(expect.objectContaining({
                cdpClientKey: "test-client-key",
                appName: "Test App",
                appLogo: "/test-logo.png",
                sessionTokenEndpoint: "/api/x402/session-token",
        ...
        it("should work without any paywall config", async () => {
            ...
            await middlewareWithoutPaywall(mockReq, mockRes, mockNext);
            expect(getPaywallHtml).toHaveBeenCalledWith(expect.objectContaining({
                sessionTokenEndpoint: undefined,
                cdpClientKey: undefined,
                appName: undefined,
