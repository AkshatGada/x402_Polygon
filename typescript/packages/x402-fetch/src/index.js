 ...
 */
export function wrapFetchWithPayment(fetch, walletClient, maxValue = BigInt(0.1 * 10 ** 6), // Default to 0.10 USDC
paymentRequirementsSelector = selectPaymentRequirements) {
    return async (input, init) => {
        ...
        let selectedPaymentRequirements = paymentRequirementsSelector(parsedPaymentRequirements, chainId ? ChainIdToNetwork[chainId] : undefined, "exact-scaled");
        // If no exact-scaled found, try exact
        if (!selectedPaymentRequirements || selectedPaymentRequirements.scheme !== "exact-scaled") {
            selectedPaymentRequirements = paymentRequirementsSelector(parsedPaymentRequirements, chainId ? ChainIdToNetwork[chainId] : undefined, "exact");
        }
