...
describe("fetchWithPayment()", () => {
    ...
    beforeEach(async () => {
        ...
        selectPaymentRequirements.mockImplementation((requirements, _) => requirements[0]);
    ...
    it("should handle 402 errors and retry with payment header", async () => {
        ...
        createPaymentHeader.mockResolvedValue(paymentHeader);
        selectPaymentRequirements.mockImplementation((requirements, _) => requirements[0]);
        mockFetch
            .mockResolvedValueOnce(createResponse(402, { accepts: validPaymentRequirements, x402Version: 1 }))
        ...
        expect(result).toBe(successResponse);
        expect(selectPaymentRequirements).toHaveBeenCalledWith(validPaymentRequirements, undefined, "exact");
        expect(createPaymentHeader).toHaveBeenCalledWith(mockWalletClient, 1, validPaymentRequirements[0]);
    ...
    it("should reject if payment header creation fails", async () => {
        ...
        createPaymentHeader.mockRejectedValue(paymentError);
        mockFetch.mockResolvedValue(createResponse(402, { accepts: validPaymentRequirements, x402Version: 1 }));
        await expect(wrappedFetch("https://api.example.com", {
            method: "GET",
