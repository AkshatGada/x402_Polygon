...
console.log(`🎯 X402-SCALED Benchmark Test - ${BATCH_SIZE} Concurrent Requests`);
console.log("═".repeat(80));
...
async function runBenchmark() {
  console.log(`⏳ Starting benchmark with ${BATCH_SIZE} concurrent requests...`);
  console.log("");

  const batchStart = Date.now();
  
  // Create array of promises
  const requests = Array.from({ length: BATCH_SIZE }, (_, i) => makeRequest(i + 1));
  ...
  if (metrics.successful > 0) {
    ...
    console.log(`   Cumulative totalValue: ~${BATCH_SIZE * 1000} wei (${(BATCH_SIZE * 0.001).toFixed(3)} USDC)`);
