...
// Wallet configurations - using all 3 wallets for testing
const wallets = [
  {
...
async function makeRequest(wallet, id) {
  try {
    ...
    const thirdwebWallet = createWallet("inApp");
    ...
    console.log(`[Wallet ${id}] Wallet connected`);
    ...
    if (body.report) {
      ...
      return { success: true, id, wallet: wallet.address, txHash: body.transaction };
    }
...
// Run all tests
console.log('Starting Thirdweb multi-wallet concurrent transaction tests...');
runAllTests().then(() => {
  console.log('\nAll tests completed!');
});
